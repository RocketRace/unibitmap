...
if __name__ == "__main__":
    ...
    parser.add_argument("out", nargs="?", default=sys.stdout, type=argparse.FileType("w"), help="Path of the resulting Python file")
    ...
    parser.add_argument("--name", "-n", default="", type=str, help="Specify the class name in resulting file")
    parser.add_argument("--tabs", "-t", action="store_true", help="Use tabs instead of 4 spaces in resulting file")
    parser.add_argument("--generate-mapping", action="store_true", help="Generate or regenerate a color mapping for the current font, if one is missing")
    ...
    args.out.write(generate_code(px.to_unicode(mapping), name=name, tabs=args.tabs, font=args.font))
